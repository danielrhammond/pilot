...
class LazyNestedModelCollectionTreeTests: XCTestCase {
    ...
    func testNestedIndex() {
        ...
        let subject = LazyNestedModelCollectionTreeNode(modelCollection: mc)
    ...
    func testIsLazy() {
        ...
        let subject = LazyNestedModelCollectionTreeNode(modelCollection: parentMC)
        let spy = TestObserver(subject)
        childMC.state = .loaded([TestM("A.2.a"), TestM("A.2.b")])
        XCTAssertNil(spy.indexPaths.first, "Shouldn't notify for child nodes when parent hasn't expanded")
    }
    ...
    func testUpdates() {
        ...
        childMC.state = .loaded([TestM("A.2.a"), TestM("A.2.b")])
        assertEqual(spy.indexPaths.first, [[0, 1]])
    }
...
private final class TestObserver {
    ...
    init<T: Observable>(_ subject: T) where T.Event == LazyNestedModelCollectionTreeNode.Event {
        self.observer = subject.observe({ [weak self] in
            self?.events.append($0)
        })
    }

    fileprivate var events = [LazyNestedModelCollectionTreeNode.Event]()
    fileprivate var indexPaths: [[IndexPath]] {
        return events.map { (event) in
            switch event {
            case .updated(let paths): return paths
            }
        }
    }
...
private final class TestM: Model, ExpressibleByStringLiteral {
    ...
    var modelId: ModelId
    var modelVersion: ModelVersion = ModelVersion.unit
}
